...
def main():
    ...
    parser.add_argument('--agent_id', type=int, default=0,
                        help='The id of the agent (e.g., 0)')
    ...
    while os.path.isdir(load_dir+f'/episode{current_ep}') and current_ep < num:
        ...
        while os.path.exists(load_dir+f'/episode{current_ep}'+f'/{file_num}.pkl'):
            ...
            # 自动检测并收集所有tcp_agent*字段
            if 'tcp_data' in data and data['tcp_data'] is not None:
                for tcp_key in sorted(data['tcp_data'].keys()):
                    if tcp_key.startswith('tcp_agent'):
                        if tcp_key not in tcp_arrays:
                            tcp_arrays[tcp_key] = []
                        tcp_arrays[tcp_key].append(data['tcp_data'][tcp_key])

            # 收集点云数据
            if 'pointcloud' in data and data['pointcloud'] is not None:
                point_cloud_arrays.append(data['pointcloud'])
    ...
    # 转换tcp数据
    for tcp_key in tcp_arrays:
        tcp_arrays[tcp_key] = np.array(tcp_arrays[tcp_key])
    # 转换点云数据
    has_pointcloud = len(point_cloud_arrays) > 0
    if has_pointcloud:
        point_cloud_arrays = np.array(point_cloud_arrays, dtype=np.float32)  # (N, n_points, 3)
        print(f'Point cloud shape: {point_cloud_arrays.shape}')
    ...
    # 自动写入所有tcp_agent*字段
    for tcp_key, tcp_data in tcp_arrays.items():
        tcp_chunk_size = (100, tcp_data.shape[1])
        zarr_data.create_dataset(tcp_key, data=tcp_data, chunks=tcp_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
        print(f'  Saved {tcp_key}: {tcp_data.shape}')
    # 写入点云数据
    if has_pointcloud:
        pc_chunk_size = (100, *point_cloud_arrays.shape[1:])  # (100, n_points, 3)
        zarr_data.create_dataset('point_cloud', data=point_cloud_arrays, chunks=pc_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
        print(f'  Saved point_cloud: {point_cloud_arrays.shape}')
