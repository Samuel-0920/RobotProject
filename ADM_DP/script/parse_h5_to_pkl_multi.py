...
def main(load_num, task_name, agent_num):
    ...
    for i in range(load_num):
        ...
        for agent_id in range(agent_num + 1):
            ...
            for j in range(min_len):
                ...
                # Add TCP data to observation if available
                tcp_data = {}
                if "extra" in res["obs"] and f"tcp_agent{agent_id}" in res["obs"]["extra"]:
                    tcp_data[f"tcp_agent{agent_id}"] = res["obs"]["extra"][f"tcp_agent{agent_id}"][j]
                # Also include the other agent's TCP for multi-agent coordination
                other_agent_id = 1 - agent_id if agent_id < 2 else agent_id
                if "extra" in res["obs"] and f"tcp_agent{other_agent_id}" in res["obs"]["extra"]:
                    tcp_data[f"tcp_agent{other_agent_id}"] = res["obs"]["extra"][f"tcp_agent{other_agent_id}"][j]
                ...
                if agent_id == agent_num:
                    step_data = dict(
                        ...
                        tcp_data=tcp_data if tcp_data else None,
                    )
                else:
                    step_data = dict(
                        pointcloud=None,
...
if __name__ == "__main__":
    ...
    main(args.load_num, args.task_name, args.agent_num)