#python运行时从上到下，先运行这个main再运行后面那个main
if __name__ == "__main__":
    import sys
    import os
    import pathlib
    #切换RoboFactory/policy/Diffusion-Policy/diffusion_policy/workspace/robotworkspace.py
    #到../RoboFactory/policy/Diffusion-Policy
    ROOT_DIR = str(pathlib.Path(__file__).parent.parent.parent)
    #把ROOT_DIR加到Python的模块搜索路径里
    #这样才能导入比如from diffusion_policy.workspace.base_workspace import BaseWorkspace
    sys.path.append(ROOT_DIR)
    #将当前脚本目录切换到../RoboFactory/policy/Diffusion-Policy
    os.chdir(ROOT_DIR)
...
class RobotWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # set seed 配置文件中的seed42会被前面的配置文件覆盖为如100
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if cfg.training.freeze_encoder:
                    #为什么这里不用with torch.no_grad()？
                    #因为torch.no_grad()会作用到整个网络 而这里只冻结encoder
