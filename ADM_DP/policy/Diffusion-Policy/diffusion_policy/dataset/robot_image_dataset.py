...
class RobotImageDataset(BaseImageDataset):
    def __init__(self,
        ...
        # <save_dir>.zarr/  
        # ├── data/  
        # │   ├── head_camera      (N, 3, H, W)    uint8 (默认)  
        # │   ├── tcp_action       (N, A)          float32  没用到
        # │   ├── state            (N, S)          float32  
        # │   └── action           (N, J)          float32  
        # └── meta/  
        #    └── episode_ends     (E,)            int64  
        #N是总帧数
        #将zarr数据集加载到ReplayBuffer中
        # 先加载基础字段，再自动检测zarr中所有tcp_agent*字段
        import zarr as zarr_lib
        zarr_root = zarr_lib.open(zarr_path, 'r')
        tcp_keys = sorted([k for k in zarr_root['data'].keys() if k.startswith('tcp_agent')])
        self.tcp_keys = tcp_keys  # 保存供后续使用
        # 自动检测点云key
        pointcloud_keys = sorted([k for k in zarr_root['data'].keys() if k == 'point_cloud'])
        ...
        # 生成验证集掩码
        #假设总的轨迹数为150, val_mask返回的长度也为150。标明哪几套轨迹用来训练
        ...
        # 初始化缓冲区，存储每个样本的序列
        #sequence_length是horizon
        sequence_length = self.sampler.sequence_length
        #这是字典推导的写法 for循环在后面
        #这样写更好理解
        #result = {}
        #for k, v in self.sampler.replay_buffer.items():
        #    result[k] = np.zeros((batch_size, sequence_length, *v.shape[1:]),dtype=v.dtype)
        #k就是head_camera, state, action
        #v就是对应的值
        #self.buffers主要用在后面getitem中，预先分配好存储空间，避免在训练时反复申请
        self.buffers = {
            k: np.zeros((batch_size, sequence_length, *v.shape[1:]), dtype=v.dtype) #v是全0的数组
            for k, v in self.sampler.replay_buffer.items()
        }
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        # 为不同的相机图像获取归一化器
        #x ∈ [0, 255] → float ∈ [0.0, 1.0](postprocess) → 再归一化为 [-1.0, 1.0]
    ...
    #似乎作者并没有用到这个函数，而是用的def postprocess函数
    #原版diffusion policy的写法是放到def __getitem__函数中
    ...
    def postprocess(self, samples, device):
        ...
        # 自动添加所有tcp_agent*字段
        for key in self.tcp_keys:
            obs_dict[key] = samples[key].to(device, non_blocking=True)  # B, T, 3
        # 自动添加点云数据
        for key in self.pointcloud_keys:
            obs_dict[key] = samples[key].to(device, non_blocking=True).float()  # B, T, N_points, 3
        # 自动添加触觉数据
        for key in self.tactile_keys:
