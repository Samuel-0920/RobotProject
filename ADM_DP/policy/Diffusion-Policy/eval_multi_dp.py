...
def get_model_input(observation, agent_pos, agent_id):
    ...
    # Auto-detect all tcp_agent* fields from observation
    for key in sorted(observation['extra'].keys()):
        if key.startswith('tcp_agent'):
            result[key] = observation['extra'][key].squeeze(0).numpy()
    # 点云数据: obs_mode="pointcloud"时在observation['pointcloud']['xyzw']中
    if 'pointcloud' in observation:
        pc = observation['pointcloud']['xyzw']  # (B, N_points, 4) — xyz + w
        if isinstance(pc, torch.Tensor):
            pc = pc.squeeze(0).numpy()
        result['point_cloud'] = pc[:, :3]  # 只取xyz, (N_points, 3)
    # 触觉数据: 每个agent自己的触觉观测
    tactile_key = f'tactile_agent{agent_id}'
    if 'extra' in observation and tactile_key in observation['extra']:
        tactile = observation['extra'][tactile_key]
        if isinstance(tactile, torch.Tensor):
            tactile = tactile.squeeze(0).numpy()
        result['tactile'] = tactile  # (32,)
    return result
